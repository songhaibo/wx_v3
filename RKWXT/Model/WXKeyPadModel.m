...
- (void)dealloc{
    ...
    RELEASE_SAFELY(_contacterFilter);
    [self removeOBS];
    [super dealloc];
}
...
- (void)loadHistory{
    ...
    for(CallHistoryEntity *entity in list){
        ...
        if(!entityExt){
            entityExt = [[[CallHistoryEntityExt alloc] init] autorelease];
            [entityExt addRecord:entity];
            ...
            if(entity){
                ...
                if(entity){
                    ...
                    StrangerEntity *stranger =  [[[StrangerEntity alloc] init] autorelease];
    ...
    for(ContacterEntity *entity in list){
        ...
        for(NSString *phone in phoneNumbers){
            if([phone rangeOfString:searchString].location != NSNotFound){
                SysContacterEntityEx *sysContacterEntityEx = [[[SysContacterEntityEx alloc] init] autorelease];
...
- (ContactBaseEntity*)callhistoryContactEntityAtRow:(NSInteger)row{
    ...
    if(!contactEntity){
        ...
        contactEntity = [[[StrangerEntity alloc] init] autorelease];
